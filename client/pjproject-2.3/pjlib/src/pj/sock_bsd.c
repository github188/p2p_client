...
const pj_uint16_t PJ_TCP_NODELAY= 1; //TCP_NODELAY; modify for p2p, android 
...
PJ_DEF(pj_status_t) pj_inet_pton(int af, const pj_str_t *src, void *dst)
{
    ...
    PJ_ASSERT_RETURN(src && src->ptr && src->slen && dst, PJ_EINVAL);
...
///#if defined(PJ_IPHONE_OS_HAS_MULTITASKING_SUPPORT) && \ //modify buy p2p
...
PJ_DEF(pj_status_t) pj_sock_close(pj_sock_t sock)
{
    ...
    defined(PJ_WIN32_WINCE) && PJ_WIN32_WINCE!=0
#if PJ_USE_IO_QUEUE_WINNT //add for p2p
