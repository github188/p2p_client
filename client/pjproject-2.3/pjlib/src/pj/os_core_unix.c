				      ...
				      pj_thread_t **ptr_thread)
{
	...
	//add by p2p
#if defined(WIN32) || defined(ANDROID_BUILD) || defined(IOS_BUILD)
#else
	if (flags & PJ_THREAD_MAX_PRIO){
		pthread_attr_setinheritsched(&thread_attr, PTHREAD_EXPLICIT_SCHED);
		pthread_attr_setschedpolicy(&thread_attr, SCHED_RR);  //SCHED_FIFO SCHED_RR
		param.sched_priority   =   99;  
		pthread_attr_setschedparam(&thread_attr, &param);  

		PJ_LOG(4, (rec->obj_name, "Thread pthread_attr_setschedparam 99"));
	}
#endif
...
static pj_status_t init_mutex(pj_mutex_t *mutex, const char *name, int type)
{
	...
	rc = pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_NORMAL);//PTHREAD_MUTEX_FAST_NP); //modify for p2p
