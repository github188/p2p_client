...
typedef struct pj_turn_sock_cb
{
	...
	//add for p2p
	void (*on_p2p_connect)(struct p2p_conn_arg* arg, 
		pj_status_t status);
	...
	pj_status_t (*on_get_local_info)(pj_turn_sock *turn_sock,
		char* remote_info,
		unsigned remote_info_len,
		char* local_info, 
		unsigned local_info_maxlen);

	pj_status_t (*on_p2p_disconnect)(pj_turn_sock *turn_sock);
...
PJ_DECL(pj_status_t) pj_turn_sock_get_sockaddr(pj_turn_sock *turn_sock,
