...
//add for p2p
typedef struct p2p_conn_arg
{
	struct p2p_transport* transport;
	pj_int32_t conn_id;
	void* user_data;
	char* remote_user;
...
typedef struct pj_turn_session_cb
{
	...
	pj_status_t (*on_get_local_info)(pj_turn_session *sess,
		char* remote_info,
		unsigned remote_info_len,
		char* local_info,
		unsigned local_info_maxlen);

	void (*on_p2p_disconnect)(pj_turn_session *sess);
} pj_turn_session_cb;
...
typedef struct pj_turn_alloc_param
{
	...
	*/
	...
	/*
	add for p2p
	only use for p2p handshake, do not transmit other data	*/
	pj_bool_t is_assist;
