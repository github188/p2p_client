...
#include <pj/pool.h>
#include <pj/rand.h>
#include <pjnath/p2p_upnp.h> //add for p2p
...
struct pj_stun_sock
{
	...
	pj_uint16_t upnp_port; //add for p2p
};
					 ...
					 pj_stun_sock **p_stun_sock)
{
    ...
    pj_status_t status;
	pj_uint16_t old_port; //add for p2p
	...
	//try use upnp port, add for p2p
	old_port = pj_sockaddr_get_port(&bound_addr);

#define UPNP_WAIT_SLEEP 0
	for(i=0; i<UPNP_ITEM_RESERVE_COUNT; i++)
	{
		if (malloc_p2p_upnp_port(&stun_sock->upnp_port))
		{
			pj_sockaddr_set_port(&bound_addr, stun_sock->upnp_port);
			status = pj_sock_bind(stun_sock->sock_fd, &bound_addr, pj_sockaddr_get_len(&bound_addr));
			if (status == PJ_SUCCESS)
				break;
			else
				stun_sock->upnp_port = 0;
		}
		else
		{
			pj_thread_sleep(UPNP_WAIT_SLEEP);
		}
	}

	if(stun_sock->upnp_port == 0)
	{
		pj_sockaddr_set_port(&bound_addr, old_port);
...
PJ_DEF(pj_status_t) pj_stun_sock_destroy(pj_stun_sock *stun_sock)
{
	...
	//add for p2p
	if(stun_sock->upnp_port != 0)
		free_p2p_upnp_port(stun_sock->upnp_port);
    if (stun_sock->stun_sess) {
	pj_stun_session_destroy(stun_sock->stun_sess);
    }
...
PJ_DECL(pj_status_t) pj_stun_sock_get_sock(pj_stun_sock *stun_sock, pj_sock_t* sock)
{
	if(stun_sock->active_sock)
	{
		*sock = stun_sock->sock_fd;
		return PJ_SUCCESS;
	}
	return PJ_EINVALIDOP;
}