...
static void stun_tsx_on_complete(pj_stun_client_tsx *tsx,
				 ...
				 unsigned transmit_count); //modify by p2p
...
static void stun_tsx_on_destroy(pj_stun_client_tsx *tsx)
{
	...
	delay_destroy_pool(tdata->pool); //modify for p2p
    }
				 ...
				 unsigned transmit_count)//modify by p2p
{
	...
	(*sess->cb.on_request_complete)(sess, status, tdata->token, tdata, 
					response, src_addr, src_addr_len, transmit_count);
    }
...
static void stun_sess_on_destroy(void *comp)
{
	...
	delay_destroy_pool(sess->rx_pool); //modify for p2p
	sess->rx_pool = NULL;
    }

    //pj_pool_release(sess->pool);
	delay_destroy_pool(sess->pool); //modify for p2p
					...
					unsigned src_addr_len)
{
	...
	//if (sess->auth_type == PJ_STUN_AUTH_NONE
	//	|| msg->hdr.type == PJ_STUN_P2P_CONNNECT_RESPONSE //add for p2p
	//	|| msg->hdr.type == PJ_STUN_P2P_EXCHANGE_INFO_RESPONSE
	//	|| msg->hdr.type == PJ_STUN_P2P_CLOSE_RELAYED_SOCKET_RESPONSE
	//	|| msg->hdr.type == PJ_STUN_P2P_REPORT_SESSION_INFO_RESPONSE)
...
//add for p2p
static pj_status_t p2p_connect_respone(pj_stun_session *sess,
									   void *token,
									   pj_pool_t *tmp_pool,
									   pj_stun_msg *msg,
									   const pj_sockaddr_t *src_addr,
									   unsigned src_addr_len)
{
	pj_status_t status;
	pj_stun_msg *response;
	pj_stun_string_attr* local_name_attr;
	...
	local_name_attr = (pj_stun_string_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_P2P_LOCAL_USER, 0);
	if(local_name_attr == NULL)
		return PJ_STUN_SC_BAD_REQUEST;
	remote_name_attr = (pj_stun_string_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_P2P_REMOTE_USER, 0);
	if(remote_name_attr == NULL)
		return PJ_STUN_SC_BAD_REQUEST;
	conn_id_attr = (pj_stun_uint_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_P2P_CONN_ID, 0);
	if(conn_id_attr == NULL)
		return PJ_STUN_SC_BAD_REQUEST;
	conn_flag_attr = (pj_stun_uint_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_P2P_TYPE, 0);
	...
	pj_stun_msg_add_uint_attr(tmp_pool, response, PJ_STUN_ATTR_P2P_TYPE, conn_flag_attr->value);
...
static pj_status_t p2p_exchange_info_respone(pj_stun_session *sess,
											 void *token,
											 pj_pool_t *tmp_pool,
											 pj_stun_msg *msg,
											 const pj_sockaddr_t *src_addr,
											 unsigned src_addr_len)
{
	...
	local_info_attr = (pj_stun_string_attr*)pj_stun_msg_find_attr(msg, PJ_STUN_ATTR_P2P_LOCAL_INFO, 0);
	if(local_info_attr == NULL)
		return PJ_STUN_SC_BAD_REQUEST;

	status = sess->cb.on_get_local_info(sess, 
		local_info_attr->value.ptr,
		local_info_attr->value.slen,
	...
	{
		info = pj_str(buffer);
		pj_stun_msg_add_string_attr(tmp_pool, response, PJ_STUN_ATTR_P2P_LOCAL_INFO, &info);
	}
...
static pj_status_t p2p_on_disconnect(pj_stun_session *sess)
{
				       ...
				       unsigned src_addr_len)
{
	...
	//add for p2p
	if(msg->hdr.type == PJ_STUN_P2P_CONNNECT_REQUEST)
		return p2p_connect_respone(sess, token, tmp_pool, msg, src_addr, src_addr_len);

	if(msg->hdr.type == PJ_STUN_P2P_DISCONNNECT_REQUEST)
	{
		char addr[256];
		pj_sockaddr_print(src_addr, addr, sizeof(addr), 3);
		PJ_LOG(4,(SNAME(sess), "on_incoming_request PJ_STUN_P2P_DISCONNNECT_REQUEST %s", addr));
		return p2p_on_disconnect(sess);
	}

	if(msg->hdr.type == PJ_STUN_P2P_EXCHANGE_INFO_REQUEST)
		return p2p_exchange_info_respone(sess, token, tmp_pool, msg, src_addr, src_addr_len);
