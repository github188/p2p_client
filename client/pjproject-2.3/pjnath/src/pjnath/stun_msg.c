...
static struct attr_desc mandatory_attr_desc[] = 
{
	...
	/*add for p2p*/
	{
		/* PJ_STUN_ATTR_P2P_TRANSMIT_COUNT, */
		"PJ_STUN_ATTR_P2P_TRANSMIT_COUNT",
		&decode_uint_attr,
		&encode_uint_attr,
		&clone_uint_attr
...
PJ_DEF(const char*) pj_stun_get_method_name(unsigned msg_type)
{
	...
	{
		switch(msg_type) //add for p2p
		{
		case PJ_STUN_P2P_CONNNECT_REQUEST:
		case PJ_STUN_P2P_CONNNECT_RESPONSE:
		case PJ_STUN_P2P_CONNNECT_ERROR_RESPONSE:
			return "p2pconnect";
		case PJ_STUN_P2P_EXCHANGE_INFO_REQUEST:
		case PJ_STUN_P2P_EXCHANGE_INFO_RESPONSE:
		case PJ_STUN_P2P_EXCHANGE_INFO_ERROR_RESPONSE:
			return "p2pexchange";
		case PJ_STUN_P2P_DISCONNNECT_REQUEST:
			return "p2pdisconnect";
		case PJ_STUN_P2P_CLOSE_RELAYED_SOCKET: 
		case PJ_STUN_P2P_CLOSE_RELAYED_SOCKET_RESPONSE:
			return "p2pcloserelayed";
		case PJ_STUN_P2P_REPORT_SESSION_INFO:
		case PJ_STUN_P2P_REPORT_SESSION_INFO_RESPONSE:
			return "p2preportinfo";
		}
		return "???";
	}
				      ...
				      unsigned options)
{
    ...
    if (*pdu != 0x00 && *pdu != 0x01 && *pdu != 0x10 && *pdu != 0x11) //modify for p2p
