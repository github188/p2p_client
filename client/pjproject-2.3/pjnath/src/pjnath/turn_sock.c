										...
										pj_status_t status)
{
	pj_turn_sock *turn_sock = (pj_turn_sock*) pj_turn_session_get_user_data(sess);
	if(turn_sock)
		turn_sock->cb.on_p2p_exchange_info(turn_sock, remote_info, status);
}



static pj_status_t turn_sock_on_get_local_info(pj_turn_session *sess,
											   char* remote_info,
											   ...
											   unsigned local_info_maxlen)
{
	pj_turn_sock *turn_sock = (pj_turn_sock*) pj_turn_session_get_user_data(sess);
	if(turn_sock)
		return turn_sock->cb.on_get_local_info(turn_sock, remote_info, remote_info_len, local_info, local_info_maxlen);
	else
		return PJ_EGONE;
}

static void turn_sock_on_p2p_disconnect(pj_turn_session *sess)
{
	pj_turn_sock *turn_sock = (pj_turn_sock*) pj_turn_session_get_user_data(sess);
	if(turn_sock)
		turn_sock->cb.on_p2p_disconnect(turn_sock);
}
/*
 * Create.
 */
					...
					pj_turn_sock **p_turn_sock)
{
	...
	sess_cb.on_p2p_connect = turn_sock->cb.on_p2p_connect; //add for p2p
	sess_cb.on_p2p_exchange_info = &turn_sock_p2p_exchange_info;
	sess_cb.on_recved_p2p_connect = &turn_sock_on_recved_p2p_connect;
	sess_cb.on_get_local_info = &turn_sock_on_get_local_info;
	sess_cb.on_p2p_disconnect = &turn_sock_on_p2p_disconnect;
...
static void timer_cb(pj_timer_heap_t *th, pj_timer_entry *e)
{
	...
	case TIMER_CONNECT_TIMEOUT:
		PJ_LOG(4,(turn_sock->pool->obj_name, "timer_cb TIMER_CONNECT_TIMEOUT"));
		if (turn_sock->cb.on_state) {//add for p2p
			(*turn_sock->cb.on_state)(turn_sock, PJ_TURN_STATE_RESOLVED, PJ_TURN_STATE_DESTROYING);
		}
		break;
    default:
				     ...
				     pj_status_t status)
{
	...
	//add by p2p, for connect long time
			  ...
			  pj_turn_state_t new_state)
{
	...
	turn_sock->sock = sock; //add for p2p
	PJ_LOG(5,(turn_sock->pool->obj_name,
		  "Connecting to %s", 
	...
	else{//add for p2p, connect long time
		pj_time_val delay = {15, 0};
		PJ_LOG(4,(turn_sock->pool->obj_name, "pj_timer_heap_schedule_w_grp_lock TIMER_CONNECT_TIMEOUT"));
		pj_timer_heap_schedule_w_grp_lock(turn_sock->cfg.timer_heap,
			&turn_sock->timer,
			&delay, TIMER_CONNECT_TIMEOUT,
			turn_sock->grp_lock);
	}
#else
	on_connect_complete(turn_sock->active_sock, PJ_SUCCESS);
...
//add for p2p
PJ_DECL(pj_status_t) pj_turn_sock_p2p_connnect(pj_turn_sock *turn_sock,
											   pj_str_t* local_user,
											   pj_str_t* remote_user,
											   p2p_conn_arg *arg)
{
	return pj_turn_session_p2p_connnect(turn_sock->sess, local_user, remote_user, arg);
}

PJ_DECL(pj_status_t) pj_turn_sock_p2p_exchange_info(pj_turn_sock *turn_sock, char* local_info)
{
	return pj_ice_session_p2p_exchange_info(turn_sock->sess, local_info);
}

PJ_DECL(pj_status_t) pj_turn_sock_get_sockaddr(pj_turn_sock *turn_sock, pj_sockaddr_t *dst_addr, int *namelen)
{
	if(turn_sock->active_sock)
	{
		return pj_sock_getsockname(turn_sock->sock, dst_addr, namelen);
	}
	return PJ_EINVALIDOP;
}

PJ_DECL(pj_status_t) pj_turn_sock_close_relayed_socket(pj_turn_sock *turn_sock)
{
	return pj_ice_session_close_relayed_socket(turn_sock->sess);
}

PJ_DECL(pj_status_t) pj_turn_sock_report_session_info(pj_turn_sock *turn_sock,
													  pj_str_t* remote_user,
...
{
	return pj_ice_session_report_session_info(turn_sock->sess,
		remote_user,
		begin_time,
		result,
		conn_id,
...
PJ_DECL(pj_status_t) pj_turn_sock_report_session_destroyed(pj_turn_sock *turn_sock,
														   pj_int32_t conn_id,
														   char* client_guid)
{
	return pj_ice_session_report_session_destroyed(turn_sock->sess,
		conn_id,
		client_guid);
}

PJ_DECL(int) pj_turn_sock_server_net_state(pj_turn_sock *turn_sock){
	if(turn_sock == 0)
		return PJ_STUN_MAX_TRANSMIT_COUNT;
	return pj_ice_session_server_net_state(turn_sock->sess);
}