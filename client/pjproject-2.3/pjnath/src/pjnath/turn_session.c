...
#define PJ_P2P_REFLUSH_TIME 60 //add for p2p
...
static void turn_session_on_p2p_disconnect(pj_stun_session *stun)
{
	pj_turn_session *sess ;
	sess = (pj_turn_session*)pj_stun_session_get_user_data(stun);
	if(sess->cb.on_p2p_disconnect)
		sess->cb.on_p2p_disconnect(sess);
}


static pj_status_t turn_session_on_get_local_info(pj_stun_session *stun, 
												  char* remote_info,
												  ...
												  unsigned local_info_maxlen)
{
	pj_turn_session *sess ;
	sess = (pj_turn_session*)pj_stun_session_get_user_data(stun);
	if(sess->cb.on_get_local_info)
		return sess->cb.on_get_local_info(sess, remote_info, remote_info_len, local_info, local_info_maxlen);
	else
		return PJ_SUCCESS;
};
					    ...
					    pj_turn_session **p_sess)
{
	...
	stun_cb.on_recved_p2p_connect = &turn_session_on_recved_p2p_connect; //add for p2p
	stun_cb.on_p2p_disconnect = &turn_session_on_p2p_disconnect; //add for p2p
	stun_cb.on_get_local_info = &turn_session_on_get_local_info; //add for p2p
...
static void turn_sess_on_destroy(void *comp)
{
	...
	sess->pool = NULL;
	//pj_pool_release(pool);
	delay_destroy_pool(pool);//modify for p2p
    }
}
					  ...
					  const pj_turn_alloc_param *param)
{
	...
	/*add for p2p*/
	pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg,
		PJ_STUN_ATTR_P2P_ASSIST,
		sess->alloc_param.is_assist?1:0);

	if(!sess->alloc_param.is_assist)
	{
		pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg,
			PJ_STUN_ATTR_P2P_INITIATIVE,
			sess->alloc_param.is_initiative?1:0);

		pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg,
			PJ_STUN_ATTR_P2P_CONN_ID,
			sess->alloc_param.conn_id);

		pj_stun_msg_add_string_attr(tdata->pool, tdata->msg,
			PJ_STUN_ATTR_P2P_REMOTE_USER,
			&sess->alloc_param.remote_user);
	}
					    ...
					    unsigned addr_len)
{
	...
	status = pj_turn_session_set_perm(sess, 1, (const pj_sockaddr*)addr, 
					  1);//set options=1, modify by p2p,when p2p relay,
	if (status != PJ_SUCCESS) {
	    pj_grp_lock_release(sess->grp_lock);
	    return status;
	}
				...
				const pj_stun_msg *msg)
{
    ...
    sess->lifetime = PJ_P2P_REFLUSH_TIME; //lf_attr->value; modify for p2p
...
//send net information to turn server
static void report_net_info_to_server(pj_turn_session* sess, unsigned transmit_count)
{
	pj_stun_tx_data *tdata;
	pj_status_t status;
	status = pj_stun_session_create_req(
		sess->stun, 
		PJ_STUN_P2P_REPORT_SESSION_INFO,
		PJ_STUN_MAGIC, NULL, &tdata);
	if (status != PJ_SUCCESS)
		return;

	status = pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_TRANSMIT_COUNT, transmit_count);
	if (status != PJ_SUCCESS) {
		pj_stun_msg_destroy_tdata(sess->stun, tdata);
		return;
	}
					 ...
					 unsigned transmit_count)
{
	...
	{
		//add for p2p
		//report_net_info_to_server(sess, transmit_count);
	    ...
	    PJ_STUN_IS_SUCCESS_RESPONSE(response->hdr.type)) 
	{
		...
		if(tdata->msg->hdr.type == PJ_STUN_P2P_CONNNECT_REQUEST){ //add for p2p		
			...
			{
				(*sess->cb.on_p2p_connect)(arg, status);
			}
		} 
		else if(tdata->msg->hdr.type == PJ_STUN_P2P_EXCHANGE_INFO_REQUEST){
			if(status == PJ_SUCCESS && response)
			{
				pj_stun_string_attr* local_info_attr;
				local_info_attr = (pj_stun_string_attr*)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_P2P_LOCAL_INFO, 0);
				if(local_info_attr)
				{
					if(sess->cb.on_p2p_exchange_info)
						(*sess->cb.on_p2p_exchange_info)(sess, &local_info_attr->value, status);
				}
			}
			else
			{
				if(sess->cb.on_p2p_exchange_info)
					(*sess->cb.on_p2p_exchange_info)(sess, 0, status);
			}
		}
		else if(tdata->msg->hdr.type == PJ_STUN_P2P_REPORT_SESSION_INFO){
			if(status == PJ_SUCCESS && response){
				pj_stun_uint_attr* transmit_count_attr = NULL;
				transmit_count_attr = (pj_stun_uint_attr*)pj_stun_msg_find_attr(response, PJ_STUN_ATTR_P2P_TRANSMIT_COUNT, 0);
				if(transmit_count_attr){
					sess->refresh_transmit_count = transmit_count_attr->value;
				}
			}
		}
		else
...
//add for p2p
PJ_DECL(pj_status_t) pj_turn_session_p2p_connnect(pj_turn_session *sess,
												  pj_str_t* local_user,
												  pj_str_t* remote_user,
												  p2p_conn_arg *arg)
{
	...
	/* Create p2p connect request */
	status = pj_stun_session_create_req(sess->stun, 
		PJ_STUN_P2P_CONNNECT_REQUEST,
		PJ_STUN_MAGIC, NULL, &tdata);
	if (status != PJ_SUCCESS)
		goto on_return;

	status = pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_CONN_ID, arg->conn_id);
	if (status != PJ_SUCCESS) {
		pj_stun_msg_destroy_tdata(sess->stun, tdata);
		goto on_return;
	}

	status = pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_TYPE, arg->conn_flag);
	if (status != PJ_SUCCESS) {
		pj_stun_msg_destroy_tdata(sess->stun, tdata);
		goto on_return;
	}

	status = pj_stun_msg_add_string_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_LOCAL_USER, local_user);
	if (status != PJ_SUCCESS) {
		pj_stun_msg_destroy_tdata(sess->stun, tdata);
		goto on_return;
	}
	...
	status = pj_stun_session_send_msg(sess->stun,
		arg, 
		PJ_FALSE,
		PJ_TRUE,
		sess->srv_addr,
...
PJ_DECL(pj_status_t) pj_ice_session_p2p_exchange_info(pj_turn_session *sess, char* local_info)
{
	pj_stun_tx_data *tdata;
	pj_status_t status;
	pj_str_t info = pj_str(local_info);

	pj_grp_lock_acquire(sess->grp_lock);

	/* Create p2p exchange information request */
	status = pj_stun_session_create_req(sess->stun, 
		PJ_STUN_P2P_EXCHANGE_INFO_REQUEST,
		PJ_STUN_MAGIC, NULL, &tdata);
	if (status != PJ_SUCCESS)
		goto on_return;

	status = pj_stun_msg_add_string_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_LOCAL_INFO, &info);
	if (status != PJ_SUCCESS) {
		pj_stun_msg_destroy_tdata(sess->stun, tdata);
		goto on_return;
	}
...
PJ_DECL(pj_status_t) pj_ice_session_close_relayed_socket(pj_turn_session *sess)
{
	pj_stun_tx_data *tdata;
	pj_status_t status;
	...
	/* Create p2p close relayed socket request */
	status = pj_stun_session_create_req(sess->stun, 
		PJ_STUN_P2P_CLOSE_RELAYED_SOCKET,
		PJ_STUN_MAGIC, NULL, &tdata);
	if (status != PJ_SUCCESS)
		goto on_return;
...
PJ_DECL(pj_status_t) pj_ice_session_report_session_info(pj_turn_session *sess,
														pj_str_t* remote_user,
														...
														pj_sockaddr* remote_addr)
{
	pj_stun_tx_data *tdata;
	pj_status_t status;
	char local_addr_name[PJ_INET6_ADDRSTRLEN+10]={0};
	char remote_addr_name[PJ_INET6_ADDRSTRLEN+10]={0};
	pj_str_t local_addr_name_str;
	pj_str_t remote_addr_name_str;
	pj_str_t guid_str = pj_str(client_guid);
	pj_time_val now;
	unsigned msec;

	pj_gettimeofday(&now);
	PJ_TIME_VAL_SUB(now, *begin_time);
	msec = PJ_TIME_VAL_MSEC(now); 

	pj_sockaddr_print(local_addr, local_addr_name, sizeof(local_addr_name), 2);
	pj_sockaddr_print(remote_addr, remote_addr_name, sizeof(remote_addr_name), 2);
	local_addr_name_str = pj_str(local_addr_name);
	remote_addr_name_str = pj_str(remote_addr_name);
	...
	status = pj_stun_msg_add_string_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_REMOTE_USER, remote_user);
	status = pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_CONN_TIME, msec);
	status = pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_CONN_RESULT, result);
	status = pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_CONN_ID, conn_id);
	status = pj_stun_msg_add_string_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_GUID, &guid_str);
	status = pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_TYPE, type);
	status = pj_stun_msg_add_string_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_LOCAL_INFO, &local_addr_name_str);
	status = pj_stun_msg_add_string_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_REMOTE_INFO, &remote_addr_name_str);
...
PJ_DECL(pj_status_t) pj_ice_session_report_session_destroyed(pj_turn_session *sess,
															 pj_int32_t conn_id,
															 char* client_guid)
{
	pj_stun_tx_data *tdata;
	...
	//session destroyed, PJ_STUN_ATTR_P2P_CONN_RESULT is  -1,
	status = pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_CONN_RESULT, (pj_uint32_t)-1);
	status = pj_stun_msg_add_uint_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_CONN_ID, conn_id);
	status = pj_stun_msg_add_string_attr(tdata->pool, tdata->msg, PJ_STUN_ATTR_P2P_GUID, &guid_str);
