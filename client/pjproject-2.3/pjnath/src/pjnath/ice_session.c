...
static void on_relayed_deley_timer(pj_timer_heap_t *th, pj_timer_entry *te); //add for p2p
				       ...
				       pj_ice_sess **p_ice)
{
	...
	//add by p2p
	pj_timer_entry_init(&ice->relayed_delay_timer, TIMER_NONE, (void*)ice, &on_relayed_deley_timer);
	ice->relayed_delay_status = RELAYED_DELAY_NONE;
	
	pj_strdup_with_null(pool, &ice->server_addr, server);
			...
			pj_status_t reason)
{
	...
	pj_timer_heap_cancel_if_active(ice->stun_cfg.timer_heap,
		&ice->relayed_delay_timer, PJ_FALSE); //add for p2p
...
static void on_relayed_deley_timer(pj_timer_heap_t *th, pj_timer_entry *te) //add for p2p
{
	pj_ice_sess *ice = (pj_ice_sess*) te->user_data;
	PJ_UNUSED_ARG(th);
	LOG4((ice->obj_name, "ICE session on_relayed_deley_timer"));
	ice->relayed_delay_status = RELAYED_DELAY_DONE;
	on_ice_complete(ice, PJ_SUCCESS);
}

static void update_cliet_status(pj_ice_sess *ice,
								...
								pj_ice_sess_comp *comp)
{
     ...
     */
	unsigned int i;
	if(check->lcand->type == PJ_ICE_CAND_TYPE_RELAYED
		|| check->rcand->type == PJ_ICE_CAND_TYPE_RELAYED)
	{
		if(ice->relayed_delay_status == RELAYED_DELAY_NONE
			||ice->relayed_delay_status == RELAYED_DELAY_TIMER)
		{
			LOG4((ice->obj_name, "ICE session update_cliet_status %d", ice->relayed_delay_status));
			return;
		}
	}
...
static void on_ice_complete(pj_ice_sess *ice, pj_status_t status)
{
	//add for p2p,relay type candidate,delay ice complete
	if(status == PJ_SUCCESS 
		&& ice->comp 
		&& !ice->is_complete
		&& ice->relayed_delay_status != RELAYED_DELAY_DONE)
	{
		pj_ice_sess_check *valid_check = ice->comp[0].valid_check;
		if(valid_check)
		{
				...
				|| valid_check->rcand->type == PJ_ICE_CAND_TYPE_RELAYED)
			{
				if(ice->relayed_delay_status == RELAYED_DELAY_NONE)
				{
					...
					LOG4((ice->obj_name, "ICE session on_ice_complete RELAYED_DELAY_NONE"));
					ice->relayed_delay_status = RELAYED_DELAY_TIMER;
					pj_timer_heap_schedule_w_grp_lock(ice->stun_cfg.timer_heap,
						&ice->relayed_delay_timer, &delay,
						TIMER_COMPLETION_CALLBACK,
						ice->grp_lock);
				}
				LOG4((ice->obj_name, "ICE session on_ice_complete relayed return"));
				return;
			}
		}
	}
	...
	if(ice->relayed_delay_status == RELAYED_DELAY_TIMER)//add for p2p
	{
		LOG4((ice->obj_name, "ICE session on_ice_complete RELAYED_DELAY_TIMER"));
		update_cliet_status(ice, ice->comp[0].valid_check, find_comp(ice, ice->comp[0].valid_check->lcand->comp_id));
		pj_timer_heap_cancel_if_active(ice->stun_cfg.timer_heap,
				&ice->relayed_delay_timer, PJ_FALSE); 
		ice->relayed_delay_status = RELAYED_DELAY_NONE;
	}
	else if(ice->relayed_delay_status == RELAYED_DELAY_DONE)
	{
		LOG4((ice->obj_name, "ICE session on_ice_complete RELAYED_DELAY_DONE"));
		update_cliet_status(ice, ice->comp[0].valid_check, find_comp(ice, ice->comp[0].valid_check->lcand->comp_id));
		ice->relayed_delay_status = RELAYED_DELAY_NONE;
	}
				   ...
				   pj_ice_sess_check *check)
{
	...
	update_cliet_status(ice, check, comp); //modify for p2p
			      ...
			      const pj_ice_sess_cand rcand[])
{
    ...
    clist = &ice->clist;
    for (i=0; i<ice->lcand_cnt; ++i) {
		//if(get_p2p_global()->only_relay && ice->lcand[i].type != PJ_ICE_CAND_TYPE_RELAYED) //add for p2p,only use relay
		//	continue;
		//if(!get_p2p_global()->enable_relay && ice->lcand[i].type == PJ_ICE_CAND_TYPE_RELAYED)//add for p2p,disable relay
		//	continue;
				  ...
				  const pj_ice_rx_check *rcheck)
{
	...
	//IPV6 client to IPV4 ipc,maybe is relayed,modify by p2p
	for (i=0; i<ice->rcand_cnt; ++i) { 
		if(ice->rcand[i].type == PJ_ICE_CAND_TYPE_RELAYED 
			&& rcheck->src_addr.addr.sa_family == ice->rcand[i].addr.addr.sa_family){
			if(pj_memcmp(pj_sockaddr_get_addr(&rcheck->src_addr.addr),
					pj_sockaddr_get_addr(&ice->rcand[i].addr.addr),
					pj_sockaddr_get_addr_len(&rcheck->src_addr.addr)) == 0)
					rcand->type = PJ_ICE_CAND_TYPE_RELAYED;
			break;
		}		
	}
	if(rcand->type == PJ_ICE_CAND_TYPE_PRFLX){
		char addr_str[PJ_INET6_ADDRSTRLEN+8];
		pj_sockaddr_print(&ice->rcand[i].addr.addr, addr_str, sizeof(addr_str), 0);
		if(strcmp(addr_str, ice->server_addr.ptr) == 0)
			rcand->type = PJ_ICE_CAND_TYPE_RELAYED;
	}
